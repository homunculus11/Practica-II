...
public class Main {
    ...
    private void run() {
        ...
        while (running) {
            printMenu("MENIU PRINCIPAL",
                    ...
                    "6. Verifică baza de date",
                    "0. Ieșire");
            String choice = prompt("Alege opșiunea: ");
    ...
    private void checkConnection() {
        if (database.testConnection()) {
            ...
            printWarning("Verifică SQL Server, Java/ConsoleApp/resources/db.properties și baza de date din SSMS.");
        }
    }
    ...
    private void reportsMenu() throws SQLException {
        ...
        System.out.println("2. Studenți per curs");
        System.out.println("3. Cursuri populare");
        System.out.println("4. Catalog cursuri");
        System.out.println("5. Prețuri cursuri");
        System.out.println("6. Înrolări studenți");
        ...
        switch (prompt("Alege exportul: ")) {
            case "1":
                saveTextFile(promptDefault("Fișier TXT", "raport-cursuri.txt"), textReport);
                break;
            case "2":
                saveTextFile(promptDefault("Fișier CSV", "raport-cursuri.csv"), report.exportCSV(selection));
                break;
            case "0":
                break;
            default:
    ...
    private void saveStudent() throws SQLException {
        ...
        LocalDate birthDate = parseDate(prompt("Data nașterii (YYYY-MM-DD, gol pentru azi): "), LocalDate.now());
    ...
    private void deleteStudent() throws SQLException {
        int id = parseInt(prompt("ID student: "), -1);
        requirePositiveId(id, "ID student");
        if (confirm("Ștergi studentul " + id + "?")) {
    ...
    private void saveProfessor() throws SQLException {
        int id = parseInt(prompt("ID (gol pentru automat): "), 0);
        String name = requireText(prompt("Nume complet: "), "Numele este obligatoriu.");
        String certification = promptDefault("Certificare", "Fără grad didactic");
    ...
    private void deleteProfessor() throws SQLException {
        int id = parseInt(prompt("ID profesor: "), -1);
        requirePositiveId(id, "ID profesor");
        if (confirm("Ștergi profesorul " + id + "?")) {
    ...
    private void saveCourse() throws SQLException {
        ...
        double price = parseDouble(prompt("Preț: "), 0);
    ...
    private void saveTextFile(String fileName, String content) {
        if (fileName == null || fileName.trim().isEmpty()) {
            printWarning("Numele fișierului este obligatoriu.");
            return;
        }
        if (content == null || content.isBlank()) {
            printWarning("Nu există conșinut de salvat.");
            return;
        }
        try (FileWriter writer = new FileWriter(fileName, StandardCharsets.UTF_8)) {
            writer.write(content);
            printSuccess("Fișier salvat: " + Paths.get(fileName).toAbsolutePath());
        } catch (InvalidPathException ex) {
            printError("Calea fișierului nu este validă: " + ex.getInput());
        } catch (SecurityException ex) {
            printError("Nu ai permisiune să salvezi în această locație.");
        } catch (IOException ex) {
            printError("Nu s-a putut salva fișierul: " + ex.getMessage());
        }
    }

    private void printStudents(List<Student> students) {
        if (students.isEmpty()) {
            printInfo("Nu există studenși.");
            return;
        }
        ...
        printTotal(students.size(), "studenți");
    }

    private void printProfessors(List<Profesor> professors) {
        if (professors.isEmpty()) {
            printInfo("Nu există profesori.");
            return;
        }
    ...
    private void printCourses(List<Curs> courses) {
        if (courses.isEmpty()) {
            printInfo("Nu există cursuri.");
            return;
        }
        printTableHeader(new int[] { 6, 42, 10, 10, 30 }, "ID", "Denumire", "Tip", "Preț", "Coordonator");
    ...
    private void printTable(TableData data, int maxWidth) {
        ...
        printTotal(data.rows.size(), "rânduri încărcate");
    }
    ...
    private void requirePositiveId(int id, String fieldName) {
        if (id <= 0) {
            throw new IllegalArgumentException(fieldName + " trebuie să fie mai mare decât 0.");
        }
    }
    ...
    private int parseInt(String value, int fallback) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Valoarea trebuie să fie un număr întreg valid.");
        }
    }
    ...
    private double parseDouble(String value, double fallback) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Valoarea trebuie să fie un număr valid.");
        }
    }
    ...
    private LocalDate parseDate(String value, LocalDate fallback) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Data trebuie să fie în formatul YYYY-MM-DD.");
        }
    }
