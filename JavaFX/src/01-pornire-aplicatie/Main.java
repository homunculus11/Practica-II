...
public class Main extends Application {
        ...
        studentList.setCellFactory(list -> new ListCell<Student>() {
            ...
            protected void updateItem(Student item, boolean empty) {
                ...
                setGraphic(empty || item == null ? null : dataRow(
                        ...
                        "ID " + item.getId() + "  |  Cursuri înscrise: " + item.getNumarInrolari()));
            }
        });
    ...
    private HBox createTopBar() {
        ...
        Button testButton = new Button("Verifică baza de date");
        ...
        themeButton.setTooltip(new Tooltip("Schimbă tema"));
    ...
    private void toggleTheme(Button themeButton) {
        ...
        if (darkMode) {
            ...
            themeButton.setText("\u2600");
            themeButton.setTooltip(new Tooltip("Schimbă la modul luminos"));
        } else {
            appRoot.getStyleClass().remove("dark-mode");
            themeButton.setText("\u263E");
            themeButton.setTooltip(new Tooltip("Schimbă la modul întunecat"));
        }
    }
    ...
    private Tab createStudentTab() {
        Button refresh = secondary("Toate înregistrările");
        refresh.setOnAction(event -> showConfirmAction("Reîncarcă studenți", "Reîncarci lista completă de studenți?", this::loadStudents));
        Button searchButton = secondary("Caută");
        searchButton.setOnAction(event -> showStudentSearchDialog());
        Button add = primary("Adaugă");
        add.setOnAction(event -> showStudentDialog(null));
        Button update = secondary("Modifică");
        update.setOnAction(event -> runSafely(() -> showStudentDialog(requireSelected(studentList, "Selectează un student pentru modificare."))));
        Button delete = danger("Șterge");
        ...
        Tab tab = new Tab("Studenți", crudLayout(
                "Studenți",
                "Alege o opțiune din bara de sus. Formularele se deschid în ferestre mici.",
                studentList,
                actionPanel("Studenți", refresh, update, add, delete, searchButton)));
    ...
    private Tab createProfesorTab() {
        Button refresh = secondary("Toate înregistrările");
        refresh.setOnAction(event -> showConfirmAction("Reîncarcă profesori", "Reîncarci lista completă de profesori?", this::loadProfesori));
        Button searchButton = secondary("Caută");
        searchButton.setOnAction(event -> showProfesorSearchDialog());
        Button add = primary("Adaugă");
        add.setOnAction(event -> showProfesorDialog(null));
        Button update = secondary("Modifică");
        update.setOnAction(event -> runSafely(() -> showProfesorDialog(requireSelected(profesorList, "Selectează un profesor pentru modificare."))));
        Button delete = danger("Șterge");
        delete.setOnAction(event -> runSafely(() -> {
        ...
        Tab tab = new Tab("Profesori", crudLayout(
                "Profesori",
                "Alege o opțiune din bara de sus. Modificarea folosește profesorul selectat.",
                profesorList,
    ...
    private Tab createCursTab() {
        Button refresh = secondary("Toate înregistrările");
        refresh.setOnAction(event -> showConfirmAction("Reîncarcă cursuri", "Reîncarci lista completă de cursuri?", this::loadCursuri));
        Button searchButton = secondary("Caută / filtrează");
        searchButton.setOnAction(event -> showCursSearchDialog());
        Button add = primary("Adaugă");
        add.setOnAction(event -> showCursDialog(null));
        Button update = secondary("Modifică");
        update.setOnAction(event -> runSafely(() -> showCursDialog(requireSelected(cursList, "Selectează un curs pentru modificare."))));
        Button delete = danger("Șterge");
        delete.setOnAction(event -> runSafely(() -> {
        ...
        Tab tab = new Tab("Cursuri", crudLayout(
                "Cursuri",
                "Alege o opțiune din bara de sus. Modificarea folosește cursul selectat.",
                cursList,
    ...
    private Tab createReportTab(Stage stage) {
        ...
        Button selectedReport = primary("Generează raportul ales");
        ...
        HBox metrics = new HBox(14,
                metricCard("Cursuri", coursesMetric, "Ofertă activă"),
                metricCard("Studenți", studentsMetric, "În baza de date"),
                metricCard("înrolări", enrollmentsMetric, "Participări cursuri"));
    ...
    private void addDatabaseTableTabs(TabPane tabs) {
        for (String tableName : tableDAO.tableNames()) {
            ...
            Button refresh = secondary("Reîncarcă tabelul");
            refresh.setOnAction(event -> loadDatabaseTable(tableName));

            VBox panel = new VBox(12,
                    sectionHeader(tableName, "Vizualizare directă din tabela SQL Server."),
                    refresh,
                    tableView);
            panel.getStyleClass().add("content-panel");
    ...
    private Tab createRaioaneLocalitatiTab() {
        ...
        VBox raioanePanel = databaseTablePanel("Raioane", "Lista raioanelor disponibile În baza de date.", raioaneTable);
        VBox localitatiPanel = databaseTablePanel("Localitati", "Localități legate de raioane.", localitatiTable);
        VBox[] panels = { raioanePanel, localitatiPanel };
        String[] tableNames = { "Raioane", "Localitati" };
        int[] currentPanel = { 0 };

        Button previous = secondary("<");
        previous.setTooltip(new Tooltip("Afișează raioane"));
        previous.setMinWidth(52);

        Button next = secondary(">");
        next.setTooltip(new Tooltip("Afișează localități"));
        next.setMinWidth(52);
        ...
        raioaneLocalitatiTab = new Tab("Raioane și Localități", page);
    ...
    private VBox databaseTablePanel(String tableName, String description, TableView<TableRowData> tableView) {
        Button refresh = secondary("Reîncarcă " + tableName);
        refresh.setOnAction(event -> loadDatabaseTable(tableName));
    ...
    private void updateLoadedTableStatus(String tableName) {
        ...
        statusLabel.setText(tableName + ": " + rows + " rânduri încărcate");
    }
    ...
    private void updateRaioaneLocalitatiTabText() {
        ...
        if (raioaneCount >= 0 && localitatiCount >= 0) {
            setTabCount(raioaneLocalitatiTab, "Raioane și Localități", raioaneCount + localitatiCount);
        } else if (raioaneCount >= 0) {
            setTabCount(raioaneLocalitatiTab, "Raioane și Localități", raioaneCount);
        } else if (localitatiCount >= 0) {
            setTabCount(raioaneLocalitatiTab, "Raioane și Localități", localitatiCount);
        }
    }
    ...
    private void showStudentSearchDialog() {
        ...
        showOptionDialog("Caută student", form(labeled("Căutare", search, "Scrie numele sau prenumele studentului.")), () -> {
            var rows = studentDAO.search(search.getText());
            studentList.setItems(FXCollections.observableArrayList(rows));
            setTabCount(studentTab, "Studenți", rows.size());
            statusLabel.setText("Căutare studenți: " + search.getText());
        });
    }
    ...
    private void showStudentDialog(Student selected) {
        ...
        showOptionDialog(selected == null ? "Adaugă student" : "Modifică student",
                form(
                        labeled("ID", id, "Opțional la adăugare."),
                        labeled("Nume complet", nume, "Câmp obligatoriu."),
                        labeled("Data nașterii", dataNasterii, "Format: 2004-05-21.")),
                () -> {
    ...
    private void showProfesorSearchDialog() {
        ...
        showOptionDialog("Caută profesor", form(labeled("Căutare", search, "Scrie nume, prenume sau certificare.")), () -> {
            ...
            statusLabel.setText("Căutare profesori: " + search.getText());
        });
    }
    ...
    private void showProfesorDialog(Profesor selected) {
        ...
        ComboBox<String> certificare = new ComboBox<>(FXCollections.observableArrayList(
                "Fără grad didactic", "Grad didactic II", "Grad didactic I", "Grad didactic superior"));
        certificare.setValue("Fără grad didactic");
        ...
        showOptionDialog(selected == null ? "Adaugă profesor" : "Modifică profesor",
                form(
                        labeled("ID", id, "Opțional la adăugare."),
                        labeled("Nume complet", nume, "Câmp obligatoriu."),
                ...
                () -> {
                    ...
                    if (selected == null) {
                        ...
                        statusLabel.setText("Profesor adăugat: " + profesor.getNume());
                    } else {
    ...
    private void showCursSearchDialog() {
        ...
        showOptionDialog("Caută sau filtrează cursuri",
                form(
                        labeled("Căutare", search, "Scrie o parte din denumire."),
                ...
                () -> {
                    ...
                    statusLabel.setText("Căutare cursuri: " + search.getText());
                });
    }
    ...
    private void showCursDialog(Curs selected) {
        ...
        showOptionDialog(selected == null ? "Adaugă curs" : "Modifică curs",
                form(
                        labeled("ID", id, "Opțional la adăugare."),
                ...
                () -> {
                    ...
                    Validator.nonNegative(price, "Preț");
                    ...
                    if (selected == null) {
                        cursDAO.save(curs);
                        statusLabel.setText("Curs adăugat: " + curs.getTitlu());
                    } else {
                        cursDAO.update(curs);
    ...
    private void verifyDatabaseWithDialog() {
        if (database.testConnection()) {
            ...
            alert.setHeaderText("Conexiune activă");
            alert.setContentText("Ești conectat la " + database.getConfig().describe() + ".");
            stylePopup(alert);
            alert.showAndWait();
            testAndLoad();
            return;
        ...
        alert.setTitle("Baza de date");
        alert.setHeaderText("Baza de date nu a fost găsită");
        alert.setContentText("Nu se poate realiza conexiunea la " + database.getConfig().describe()
                + ". Verifică SQL Server, db.properties și baza de date din SSMS.");
        stylePopup(alert);
        alert.showAndWait();
    }
    ...
    private void updateStudentStatus() {
        ...
        double averageEnrollments = studentList.getItems().stream()
                ...
                .average()
                .orElse(0);
        statusLabel.setText(String.format("Studenți: %d înregistrări | Medie cursuri înscrise: %.1f", students, averageEnrollments));
    }
    ...
    private void updateProfesorStatus() {
        ...
        double averageRating = profesorList.getItems().stream()
                ...
                .average()
                .orElse(0);
        statusLabel.setText(String.format("Profesori: %d înregistrări | Rating mediu: %.1f", professors, averageRating));
    }
    ...
    private void updateCursStatus() {
        ...
        double averagePrice = cursList.getItems().stream()
                ...
                .average()
                .orElse(0);
        statusLabel.setText(String.format("Cursuri: %d înregistrări | Preț mediu: %.2f lei", courses, averagePrice));
    }

    private void updateReportStatus() {
        String text = String.format("Rapoarte: %s cursuri | %s studenți | %s Înrolări",
    ...
    private void export(Stage stage, String defaultName, String content) throws IOException {
        if (content == null || content.isBlank()) {
            throw new IOException("Nu există conșinut de exportat.");
        }
        ...
        if (file.isDirectory()) {
            throw new IOException("Alege un fișier, nu un folder.");
        }
    ...
    private boolean confirmDelete(String title, String message) {
        ...
        alert.setHeaderText("Confirmare necesară");
    ...
    private int parseId(String text) throws ValidationException {
        ...
        try {
            ...
            throw new ValidationException("ID-ul trebuie să fie un număr întreg valid.");
        }
    }

    private double parsePrice(String text) throws ValidationException {
        if (text == null || text.trim().isEmpty()) {
            throw new ValidationException("Prețul este obligatoriu.");
        }
        try {
            return Double.parseDouble(text.trim());
        } catch (Exception ex) {
            throw new ValidationException("Prețul trebuie să fie un număr valid.");
        }
    }
    ...
    private void runSafely(CheckedRunnable action) {
        try {
            ...
            showError("Eroare bază de date", AppErrors.databaseMessage(ex));
        } catch (IOException ex) {
            showError("Eroare fișier", AppErrors.userMessage(ex));
    ...
    private void showError(String title, String message) {
        String safeMessage = message == null || message.isBlank() ? "Operația nu a reușit." : message;
